/*
 * Author:  xioumu
 * Created Time:  2012-3-8 14:36:44
...
void updata(int t, int lc, int rc, int ll, int rr){
    int mid = (ll+rr) >> 1;
    if(cut[t] >= 1){
        sum[lc] = num[mid] - num[ll-1];
        sum[rc] = num[rr] - num[mid];
    }
    else{
        sum[lc] = sum[rc] = 0;
    }
...
void add(int t, int ll, int rr, int l, int r, int h){
    ...
    if(rr < l || r < ll) return;
    getch(t, lc, rc);
    //printf("%d %d %d %d %.2f %.2f %d\n",l,r,ll,rr,num[ll-1],num[rr],sum[t]);
    if(l <= ll && rr <= r){
        cut[t] += h;
        if(cut[t] >= 1){
        ...
        //printf("%d %d %d %d %.2f %.2f %.2f %d\n",l,r,t,rr,sum[lc],sum[rc],sum[t],cut[t]);
        return ;
    }
    mid = (ll + rr) >> 1;
    add(lc, ll, mid, l, r, h);
